...
func main() {
	...
	var (
		...
		prometheusIncludeDefaultMetrics = flag.Bool("prometheus-include-default-metrics", true, "Include Go runtime, process, and HTTP handler metrics in the Prometheus endpoint")
	...
	case "prometheus":
		...
		if !*prometheusIncludeDefaultMetrics {
			prom = backend.NewPrometheusBackendWithoutDefaultMetrics()
		}
